...
export default function AdOverlayMaker({ onBack }: { onBack?: () => void }) {
  ...
  const [line1a, setLine1a] = useState("지금 대구");
  const [line1b, setLine1b] = useState("이 곳이 뜬다");
  const [size1, setSize1] = useState(52);
  const [line2, setLine2] = useState("주말 핫플 추천");
  const [line2Style, setLine2Style] = useState<Line2Style>("box");
  const [size2, setSize2] = useState(48);
  const [sub, setSub] = useState("MK LINK가 직접 다녀온 대구 로컬 명소를 모았습니다.");
  const [credit, setCredit] = useState("© MK LINK");
  const [badge, setBadge] = useState("MK LINK");
