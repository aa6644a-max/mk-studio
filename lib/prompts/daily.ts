...
function buildPdfInfoPrompt(
  ...
  const user = `
        ...
        [🚨 상단 제목 규칙 — 절대 준수]
        - 글 맨 위에 카테고리 라벨/제목 헤더를 둘 경우, 제목(h1)은 핵심 주제를 담은 **2~6단어의 짧은 카테고리형 제목**이어야 합니다. (예: "DIMF 총정리", "범죄도시5 프리뷰", "청년 월세지원 안내")
        - 🚫 제목에 요약 문장·SEO 제목·줄거리·설명을 절대 넣지 마세요. 긴 설명은 제목이 아니라 본문 첫 도입 단락에 풀어 쓰세요.
        - 레퍼런스 과거글의 헤더가 길더라도 따라하지 말고 위 규칙대로 짧게.

        [🚨 본론 구성 — 내용 성격을 먼저 판단]
        PDF 내용의 성격을 먼저 분류하고, 그에 맞는 방식으로 쓰세요. 모든 글을 표로 채우는 것이 목적이 아닙니다.

        ▶ (가) 객관 정보형 — 공고·일정·제원·통계·혜택·절차·가격 등 항목/수치 위주:
           정보 시각 블록을 적극 활용합니다. 2~4개 의미 단위로 나눠 [섹션 헤더(A)]로 열고, 성격에 맞는 블록을 고르세요.
           - 수치·날짜·금액·조건·제원 등 항목형 → [2열 정보 테이블(B)] (줄글 나열 금지)
           - 꼭 기억할 핵심 → [콜아웃(C)] / 주의·예외 → [주의 박스(D)] / 절차·순서 → [단계 플로우(E)] / 비교 → [비교 카드(F)] / 핵심 문장 → [강조 인용(G)]
           - 3종류 이상 블록을 섞어 단조로움을 피하고, 블록 사이는 2~3문장으로 연결.

        ▶ (나) 서사·해설형 — 영화 프리뷰, 인물 소개, 뉴스/이슈 해설, 감상 등 흐름이 중요한 글:
           🚫 표로 욱여넣지 마세요. 줄글의 자연스러운 흐름이 핵심입니다.
           - [섹션 헤더(A)]로 단락을 나누되, 본문은 2~3문장 호흡의 줄글로 전개.
           - 강조는 [강조 인용(G)]과 [콜아웃(C)] 위주로만. 정말 항목형 메타데이터(개봉일·감독·출연, 기본 정보 등)가 있을 때만 [2열 테이블(B)]을 1개 정도 제한적으로.
           - 단계 플로우·비교 카드 등은 해당 내용이 실제로 있을 때만.

        ※ 한 PDF 안에 두 성격이 섞여 있으면 섹션별로 (가)/(나)를 다르게 적용하세요.
...
4. 마무리: 어떤 사람에게 추천할 만한지 솔직하게.`;
    ...
    case "제품리뷰":
      return `[🔑 글 구조 — 제품·리뷰]
1. 인트로: 이 제품을 쓰게 된 계기/구매 이유 한두 문장. 인사말 금지. 과장 광고 톤('인생템', '필수템') 금지.
2. 제품 개요: 무엇인지·핵심 스펙·가격대를 사진 메모 순서대로 간결하게. 정보형 항목은 💡 박스나 표로.
3. 사용 경험: 직접 써보며 좋았던 점과 아쉬웠던 점을 솔직하게 나눠서. "좋아요" 대신 "이런 점이 편했다/걸렸다"로 구체적으로.
4. 추천 대상 / 비추 대상: 어떤 사람에게 맞고 안 맞는지 솔직하게.
5. 💡 팁 박스 1~2개 (구매 팁, 관리법, 대안 제품 등).
6. 마무리: 재구매 의향 또는 총평을 담백하게.`;
