...
export function renderArticle(run: Pick<Run, "article" | "sources" | "brief">, titleIndex = 0): string {
  const a = run.article;
  if (!a) return "";
  const used = new Set(a.sections.flatMap(s => s.sourceIds));
  const references = run.sources.filter(s => used.has(s.id));
  const body = a.sections.map(s => `${s.heading ? `<table width="100%" border="0" cellpadding="15" bgcolor="#1a2e4a" style="margin:28px 0 18px"><tr><td><b style="color:#fff;font-size:18px">${escapeHtml(s.heading)}</b></td></tr></table>` : ""}
${s.paragraphs.map(p => `<p style="margin:16px 0;line-height:1.9">${rich(p)}</p>`).join("\n")}
...
export function lintArticle(run: Run): Issue[] {
  ...
  for (const section of a.sections) {
    ...
    for (const id of section.imageIds) if (!images.some(p => p.id === id)) add("format", "첨부하지 않은 사진을 참조했습니다.", section.id);
